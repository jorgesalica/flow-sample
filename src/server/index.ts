...
async function startServer() {
    ...
    const hasDistFolder = await fs.access(distPath).then(() => true).catch(() => false);
    ...
    serve({
        ...
        if (!hasDistFolder) {
            log.info('UI dist not found. Run "cd ui && npm run build" for production, or use Vite dev server.');
        }
    });
}
