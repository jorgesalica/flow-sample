...
| Security/quality contract | Node check scripts               | sensitive values, CI, hooks, PR QA, and dependency policy                        |
...
- Browser journeys avoid live provider calls and isolate mutable API state where it is
  under test. They run without credentials or network-dependent assertions.
- Sensitive-data tests use unmistakably fictional values; the scanner checks all tracked
  and unignored text while excluding only its own detection fixtures and generated files.
...
pnpm verify                              # canonical local static/unit gate; requires build outputs
pnpm test:tooling                        # architecture, runtime, coverage, and quality contracts
pnpm test:sensitive                      # scanner tests + repository scan
...
pnpm clean:artifacts                     # generated build/coverage/browser cleanup
...
CI runs a clean frozen install and two required jobs. The quality job audits production
dependencies, builds and imports compiled packages without filesystem side effects, runs
the canonical `verify` contract, and enforces every package coverage ratchet. The browser
job builds the same workspace, installs pinned Chromium support, and runs the full
Playwright suite with isolated fixtures. Failed browser runs retain a short-lived report
artifact for diagnosis.

Playwright uses one worker and no retries because all journeys target one local backend
and SQLite composition. This makes ordering explicit and ensures a flaky first attempt
fails the gate instead of being hidden by retry behavior.

Run `pnpm clean:artifacts` after local full-gate verification when generated output is no
longer needed. It removes build, coverage, and Playwright artifacts but never `.env`,
`data/`, or `outputs/` user state. A subsequent verification or push starts with
`pnpm build`; UI `check` then runs `svelte-kit sync` itself instead of relying on stale
