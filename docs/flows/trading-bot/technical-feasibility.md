...
- **Source**: Binance Public Data Stream (WebSocket).
  - _Pros_: Zero latency, no API limits for public streams, efficient pushes.
  - _Cons_: Connection management (needs reconnection logic).
- **Persistence**: SQLite (Separate instance: `trading.db`).
  - _Strategy_: Store OHLCV (Open, High, Low, Close, Volume) candles.
  - _Granularity_: 1-second ticks aggregated into 1-minute candles for storage efficiency.
...
- **Strategy**: Tiered Multi-Agent Approach.
  ...
  - **Tier 2 (Tactical Feedback)**: **Groq (Llama 3)** or **Gemini Flash**.
    - _Freq_: Every 15-60 mins.
    - _Task_: "Review the last hour of market structure. Summarize trend strength."
    - _Cost_: Extremely low (~$0.10/million tokens).
  - **Tier 3 (Strategic Mentor)**: **GPT-4o** or **Claude 3.5 Sonnet**.
    - _Freq_: Daily / On-Demand.
    - _Task_: "Deep dive analysis of the day's performance and educational feedback."
    - _Cost_: Higher, but used sparingly.
...
_Note: Costs scale with frequency of LLM calls. The "Paper Trading" POC will be effectively free if using local models (Ollama) or lower tiers._
