...
* *Logic*: If $H > 0.6$ (Trend), `Enable_Strategies = [MACD_Cross, Breakouts]`. If $H < 0.4$ (Range), `Enable_Strategies = [RSI_Div, Fakes]`.
...
* *Logic*: Breakout valid ONLY IF `Close > Level` AND `Vol > Avg`. Else = **Fakeout**.
...
* **Divergence**: Does Momentum agree with Price?
  ...
  * *Hidden*: Trend Fuel.
* *Logic*: Signal valid ONLY IF `HTF_Direction == LTF_Direction`.
