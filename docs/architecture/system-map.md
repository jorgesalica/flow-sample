...
1. **Spotify and Lyrics (#71)**: move orchestration out of routes, make response contracts
   deliberate, and remove unsafe UI boundary casts.
2. **Trading (#72)**: extract wizard/provider orchestration and type the full route
   contract.
3. **Quality gate (#73)**: ratchet coverage, complete package participation, consolidate
   exact tooling duplication, and enforce the repaired boundaries.
4. **Board persistence (#44)**: add named server-backed boards and migrate the local v1
