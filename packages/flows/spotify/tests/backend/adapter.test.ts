...
function tokenResponse(over: Partial<{ access_token: string; expires_in: number; refresh_token: string }> = {}) {
    return {
        data: {
    ...
    describe('fetchArtistDetails', () => {
        ...
        it('returns cached entries without hitting the API', async () => {
            artistCacheGetMany.mockReturnValue({
                cached: new Map([['artist-1', { genres: ['rock'], imageUrl: 'https://img.mock.test/a.jpg' }]]),
                misses: [],
            });
        ...
        it('fetches misses individually and caches the results', async () => {
            ...
            expect(artistCacheSet).toHaveBeenCalledWith('artist-2', ['jazz'], 'https://img.mock.test/160.jpg');
        });
