...
describe('SQLiteLyricsRepository', () => {
  ...
  describe('save', () => {
    it('inserts a new lyrics row with status found', async () => {
      ...
      const row = testDb.prepare('SELECT * FROM lyrics WHERE track_id = ?').get('t1') as {
        status: string;
        plain_lyrics: string;
      };
    ...
    it('upserts existing row on conflict', async () => {
      ...
      const count = (
        testDb.prepare('SELECT COUNT(*) as c FROM lyrics WHERE track_id = ?').get('t1') as {
          c: number;
        }
      ).c;
      expect(count).toBe(1);
  ...
  describe('getStats', () => {
    beforeEach(() => {
      testDb.prepare(`INSERT INTO tracks (id, title) VALUES (?, ?)`).run('t1', 'Song 1');
      testDb.prepare(`INSERT INTO tracks (id, title) VALUES (?, ?)`).run('t2', 'Song 2');
      testDb.prepare(`INSERT INTO tracks (id, title) VALUES (?, ?)`).run('t3', 'Song 3');
    });
