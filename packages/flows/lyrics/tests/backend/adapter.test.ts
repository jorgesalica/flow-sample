...
describe('LrcLibAdapter', () => {
    ...
    describe('fetchLyricsBatch', () => {
        const tracks: LyricsTrackParams[] = [
            { trackId: 'a', trackName: 'Song A', artistName: 'Artist', albumName: 'Album', durationSeconds: 180 },
            { trackId: 'b', trackName: 'Song B', artistName: 'Artist', albumName: 'Album', durationSeconds: 200 },
            { trackId: 'c', trackName: 'Song C', artistName: 'Artist', albumName: 'Album', durationSeconds: 210 },
        ];

        it('returns results for all tracks', async () => {
            getInnerClient().get
                .mockResolvedValueOnce({ data: { plainLyrics: 'Lyrics A', syncedLyrics: null, instrumental: false } })
                .mockResolvedValueOnce({ data: { plainLyrics: null, syncedLyrics: null, instrumental: true } })
                .mockResolvedValueOnce({ data: { plainLyrics: 'Lyrics C', syncedLyrics: '[00:00] C', instrumental: false } });
        ...
        it('captures per-track errors without failing the batch', async () => {
            ...
            getInnerClient().get
                ...
                .mockResolvedValueOnce({ data: { plainLyrics: 'Lyrics C', syncedLyrics: null, instrumental: false } });
