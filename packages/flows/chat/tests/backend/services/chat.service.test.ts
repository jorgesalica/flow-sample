...
function streamEvents(parts: string[], response: LLMResponse): LLMStreamEvent[] {
  return [...parts.map((delta) => ({ delta, done: false })), { delta: '', done: true, response }];
}
...
function lastDoneEvent(events: ChatStreamEvent[]): Extract<ChatStreamEvent, { type: 'done' }> {
...
describe('ChatService', () => {
  ...
  describe('conversation reads/deletes', () => {
    ...
    it('deleteConversation rejects an unknown conversation', () => {
      expect(() => service.deleteConversation('missing')).toThrow(ConversationNotFoundError);
    });
  });
  ...
  describe('sendMessage', () => {
    ...
    it('propagates errors and does not persist an assistant message on failure', async () => {
      ...
      await expect(service.sendMessage(CONV_ID, 'Hi', 'specific', 'groq:llama')).rejects.toThrow(
        'provider exploded',
      );
  ...
  describe('sendMessageStream', () => {
    it('emits user_message, deltas, then done with the saved assistant message', async () => {
      ...
      generateStreamForProvider.mockReturnValue(toAsyncGen(streamEvents(['Hel', 'lo!'], resp)));
    ...
    it('saves the user message before streaming begins', async () => {
      generateStreamForProvider.mockReturnValue(toAsyncGen(streamEvents(['hi'], okResponse())));
    ...
    it('builds the streaming request with system prompt + history', async () => {
      generateStreamForProvider.mockReturnValue(toAsyncGen(streamEvents(['x'], okResponse())));
