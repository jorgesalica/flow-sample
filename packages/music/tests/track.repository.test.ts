...
describe('SQLiteTrackRepository', () => {
  ...
  describe('save', () => {
    ...
    it('falls back to default labels for empty title/album/artist names', async () => {
      ...
      const trackRow = testDb
        .prepare('SELECT title, album_name FROM tracks WHERE id = ?')
        .get('t1') as {
        title: string;
        album_name: string;
      };
    ...
    it('upserts on id conflict (ON CONFLICT DO UPDATE) without duplicating rows', async () => {
      ...
      const row = testDb
        .prepare('SELECT title, duration_ms FROM tracks WHERE id = ?')
        .get('t1') as {
        title: string;
        duration_ms: number;
      };
    ...
    it('persists multiple tracks in a single call', async () => {
      await repo.save([makeTrack({ id: 't1' }), makeTrack({ id: 't2', title: 'Second' })]);
