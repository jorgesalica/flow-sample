...
describe('OpenAICompatibleProvider', () => {
  ...
  describe('generate()', () => {
    ...
    it('merges extraHeaders into request headers', async () => {
      mockFetch.mockResolvedValue(makeJsonResponse(MOCK_JSON_RESPONSE));
      await new TestProvider('key', { 'HTTP-Referer': 'https://myapp.com' }).generate({
        messages: [],
      });
    ...
    it('uses request model as fallback when response.model is absent', async () => {
      mockFetch.mockResolvedValue(
        makeJsonResponse({ choices: [{ message: { content: 'hi' } }], usage: {} }),
      );
  ...
  describe('generateStream()', () => {
    it('calls the correct endpoint', async () => {
      mockFetch.mockResolvedValue(
        makeStreamResponse([sseData({ choices: [{ delta: { content: 'hi' } }] })]),
      );
    ...
    it('includes extraHeaders in stream request', async () => {
      mockFetch.mockResolvedValue(
        makeStreamResponse([sseData({ choices: [{ delta: { content: 'x' } }] })]),
      );
      await collectStream(
        new TestProvider('key', { 'X-Custom': 'value' }).generateStream({ messages: [] }),
      );
